...
export function WorkspaceActionCard(props: {
  ...
  return (
    <div className="WorkspaceActionCardWrapper">
      <div
        className={
          !card.closed ? 'WorkspaceActionCard' : 'WorkspaceActionCard closed'
        }
      >
        <div
          className="top"
        ...
        {!card.closed &&
          card.dueDate &&
          card.startDate &&
          (card.submitState || !card.submitState) && (
            <div>
              <div className="card-status">
                {card.submitState === 'submitted' && (
                  <span className="submitted">SUBMITTED</span>
                )}
                {card.submitState === 'saved' && (
                  <span className="inprogress-submit">PROGRESS SAVED</span>
                )}
                {card.submitState === 'none' && (
                  <span className="no-submit">NO SUBMISSION</span>
                )}
                {card.submitState === 'waitForFeedback' && (
                ...
                {card.submitState === 'feedback' && (
                  <div
                    className={
                      card.feedback! > 0 ? 'feedback-new' : 'feedback-none'
                    }
                  >
              ...
              {card.submitState !== 'waitForFeedback' &&
                card.submitState !== 'feedback' && [
                  <Progress
                    ...
                    status={
                      card.submitState === 'submitted' ? 'success' : undefined
                    }
                    className={
                      card.submitState === 'submitted'
                        ? 'progressBar success'
                  ...
                  <div className="card-dates">
                    ...
                    >
                      {' ' +
                        ...
                        ' ' +
                        card.dueDate.toLocaleTimeString()}
                    </span>
                    <span className="days-left">
                      {calcDaysLeft(card.dueDate, card.startDate)} DAYS LEFT
                    </span>
                  </div>
                ]}
            </div>
          ...
          <div className="btn-wrapper">
            {card.active === true ? (
              <Button
                type="primary-outline"
                size="large"
                onClick={card.buttonHandleClick}
              >
                {card.buttonTitle}
              </Button>
            ) : (
